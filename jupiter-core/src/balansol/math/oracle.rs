...
pub fn calc_starting_lpt(amounts_in: Vec<u64>, weights: Vec<u64>) -> Option<u64> {
  let mut lpt_amount = weights.len().to_f64()?;
  for idx in 0..amounts_in.len() {
    let weight = calc_normalize_weight(idx, weights.clone())?;
...
pub fn calc_received_lpt_amount_after_add_full_side(
  ...
  for idx in 0..amounts_in.len() {
    let normalize_weight = calc_normalize_weight(idx, weights.clone())?;
 ...
 *****************************************************************************************/
pub fn calc_received_mint_amount_after_remove_single_side(
  lpt_amount: u64,
  supply_lpt: u64,
...
pub fn calc_received_mint_amounts_after_remove_full_side(
  lpt_amount: u64,
  supply: u64,
  reserves: Vec<u64>,
) -> Option<Vec<u64>> {
